...
namespace Punkty_przeciec
{
	public class AVLTree
	{
		...
		public Node insert(Node node, Sector key)
		{

			if (node == null)
				return (new Node(key));

			if (key.End.y < node.key.End.y)
				node.left = insert(node.left, key);
			else if (key.End.y > node.key.End.y)
			...
			if (balance > 1 && key.End.y < node.left.key.End.y)
				return rightRotate(node);

			if (balance < -1 && key.End.y > node.right.key.End.y)
		...
		public Node deleteNode(Node root, Sector key)
		{
			if (root == null)
				return root;

			if (key.End.y < root.key.End.y)
				root.left = deleteNode(root.left, key);

			else if (key.End.y > root.key.End.y)
